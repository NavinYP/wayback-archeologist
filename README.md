...
- Download media using host-aware strategies.
- Resurrect media from an archive-style host (e.g., via the Internet Archive).
- Split and dedupe the final corpus into human-sized, random batches.

The design is intentionally simple:

- Coordination happens through the filesystem (`links.txt`, `status.json`, `raw/`, `split/`).
- Each stage is its own process you can start and stop independently.
- All sensitive credentials live in `.env` and are never committed.

This repo is the **public, anonymized companion** to the `Wayback Archeologist` article. It focuses on architecture and patterns, not on scraping any specific site.
...
All services communicate only through the filesystem. There is no database, no message queue, and no external state beyond `status.json`, `manifest.json`, and the shared `proxies_cached.json`.
...
Proxy pool (optional, for archive-heavy workloads):
...
You can start/stop these processes independently. Each will pick up where it left off by reading `status.json` and `manifest.json`.

---

### Ethics & Responsible Use

This repo is **not** a how‑to guide for ignoring terms of service. It’s a reference architecture for responsible, one‑shot archival of already‑public content.

Please read `ETHICS.md` before using any of this code, and adapt the principles to your own project. In particular:

- Only access content you are allowed to see in the normal UI.
- Use conservative rate limits and respect `Retry-After` headers.
...
Choose an open-source license appropriate for your use (e.g., MIT, Apache 2.0) and include it here.
